...
def ensure_session_for_account(account_idx: int, account: dict, force_new: bool = False):
    """
    确保指定账号的会话有效
    :param force_new: 是否强制创建新会话（用于新对话开始时）
    """
    print(f"[DEBUG][ensure_session_for_account] 开始 - 账号索引: {account_idx}, 强制新会话: {force_new}")
    start_time = time.time()
    ...
    with account_manager.lock:
        ...
        # 如果强制新会话，先清除当前会话
        if force_new:
            print(f"[DEBUG][ensure_session_for_account] 强制重置会话...")
            state["session"] = None
...
def chat_completions():
    try:
        ...
        if prompts and not messages:
            if len(prompts) == 1 and prompts[0].get('role') == 'user':
                is_new_conversation = True
            else:
                is_new_conversation = False

        if is_new_conversation:
            print(f"[会话管理] 检测到新对话 (消息数: {len(messages)})，准备强制重置会话")
        ...
        if specified_account_id is not None:
            ...
            try:
                session, jwt, team_id = ensure_session_for_account(account_idx, account, force_new=is_new_conversation)
                proxy = get_proxy()
