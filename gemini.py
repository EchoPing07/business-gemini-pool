...
GENERIC_ERROR_COOLDOWN_SECONDS = 120
...
def create_admin_token(exp_seconds: int = 86400) -> str:
    payload = {"exp": time.time() + exp_seconds, "ts": int(time.time())}
...
def check_proxy(proxy: str) -> bool:
    ...
    try:
        ...
        resp = requests.get("https://www.google.com", proxies=proxies, verify=False, timeout=10)
...
def ensure_session_for_account(account_idx: int, account: dict):
    ...
    session = create_chat_session(jwt, team_id, proxy, account)
...
def stream_chat_realtime(jwt: str, sess_name: str, message: str, 
    ...
    try:
        ...
        error_chunk = {
            ...
            "choices": [{"index": 0, "delta": {"content": f"[请求错误: {e}]"}, "finish_reason": None}]
        }
    ...
    if resp.status_code != 200:
        ...
        error_chunk = {
            ...
            "model": model_name,
            "account_csesidx": account_csesidx,
            "choices": [{"index": 0, "delta": {"content": f"[API错误: {error_text}]"}, "finish_reason": None}]
        }
        yield f"data: {json.dumps(error_chunk, ensure_ascii=False)}\n\n"
        yield "data: [DONE]\n\n"
        return
...
def chat_completions():
    try:
        ...
        requested_model = data.get('model', 'gemini-enterprise')
        include_thought = data.get('include_thought', False)
        models_config = account_manager.config.get("models", [])
        models_mapping = {m.get("id", ""): m.get("id", "") for m in models_config}
...
def logging_config():
    if request.method == 'GET':
        return jsonify({"level": CURRENT_LOG_LEVEL_NAME, "levels": list(LOG_LEVELS.keys())})
    data = request.json or {}
...
def admin_login():
    ...
    resp.set_cookie("admin_token", token, max_age=86400, httponly=True, secure=False, samesite="Lax", path="/")
    return resp
...
def test_proxy():
    ...
    return jsonify({"success": available, "message": "代理可用" if available else "代理不可用或连接超时"})
...
def get_proxy_status():
    ...
    return jsonify({"enabled": True, "url": proxy, "available": available})
